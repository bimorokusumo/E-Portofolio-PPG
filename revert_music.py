...
# Remove the YouTube widget
yt_regex = r'<!-- YouTube BGM Player -->.*?</script>'
text = re.sub(yt_regex, '', text, flags=re.DOTALL)
...
if '<!-- Floating Spotify Player for BGM -->' not in text:
    text = text.replace('</body>', spotify_widget + '\n</body>')

with open('index.html', 'w', encoding='utf-8') as f:
    f.write(text)

print("Reverted to Spotify.")
